...
vmapped_gauss_pdf = torch.vmap(func=gauss_pdf, in_dims=(None, 0, None), out_dims=(0))
delta_value = lambda x, y: x - y
vmapped_delta_value = torch.vmap(func=delta_value, in_dims=(None, 0))
...
vmapped_gauss_log_pdf = torch.vmap(func=gauss_log_pdf, in_dims=(None, 0, None), out_dims=(0))
...
vmapped_gauss_score = torch.vmap(func=gauss_score, in_dims=(None, 0, None), out_dims=(0))
...
vammped_parzen_log_pdf = torch.vmap(func=parzen_log_pdf, in_dims=(0, None, None), out_dims=0)
# the score function (just for validation)
score_parzen = torch.func.grad(parzen_log_pdf, argnums=(0))
vmapped_score_parzen = torch.vmap(func=score_parzen, in_dims=(0, None, None))
...
vammped_parzen_score = torch.vmap(func=parzen_score, in_dims=(0, None, None), out_dims=0)
